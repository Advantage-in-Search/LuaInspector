local env: table = assert(getgenv, 'Your exploit is not supported')();assert(isluau(),"LuaInspector is designed for LuaU only. If you're using a different executor, the script may not work as intended.");
local url: string = "https://raw.githubusercontent.com/Advantage-in-Search/LuaInspector/revision/"
local hint: Instance = Instance.new("Hint")
if not utils then
